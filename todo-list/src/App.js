...
function App() {
	...
	function toggleAllCompleted() {
		...
		const completedTasks = tasks.map((task) => ({ ...task, completed: !isAllComplete }));
		setTasks(completedTasks);
	};
		...
		<>
			...
			<div className="section-list">
				{ tasks.length >= 1 ? (
					...
					<label htmlFor="all-label" className="toggle-all-label"></label>
					</>
				) : null }
			...
			{showFilters || tasks.length >= 1 ? (
				<section className="section-filters">
					<span className="todo-count">
						{tasks.filter(FILTER_MAP["Active"]).length} {tasks.filter(FILTER_MAP["Active"]).length === 1 ? "item" : "items"}{" "}
						left
					</span>
					<ul className="todo-filters">{filterList}</ul>
            			<button className="clear-btn" id="clearAll" onClick={deleteCompleted}>
					  		{tasks.filter(FILTER_MAP["Completed"]).length >= 1 ? "Clear completed" : null}
					  	</button>
				</section>
			) : null}
			<div className="info">
